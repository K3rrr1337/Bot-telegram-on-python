...
async def add_config_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['config_name'] = None
    context.user_data['config_description'] = None
    context.user_data['config_price'] = None
    ...
    if context.user_data.get('adding_config'):
        ...
        if step == 'name':
            ...
            if update.message.document:
                ...
                config_name = context.user_data.get('config_name', '').strip()
                if not config_name:
                    await update.message.reply_text("❌ Ошибка: название товара не может быть пустым!")
                    context.user_data.clear()
                    return
                ...
                try:
                    config_id = add_config(
                        ...
                        context.user_data.get('config_price', 0),
                        'coins',
                        file_path,
                    ...
                    await update.message.reply_text(
                        ...
                        f"Цена: {context.user_data.get('config_price', 0)} монет"
                    )
                except Exception as e:
