# bot.py (финальная версия с переменными окружения)
import sqlite3
import logging
...
import random
import string
import ast  # Добавлен для парсинга ADMIN_IDS
...
# Конфигурация - теперь через переменные окружения
...
def add_config(name, description, price, price_type, file_path, photo_path=None):
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (name, description, price, price_type, file_path, photo_path))
    conn.commit()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('adding_config'):
        ...
        if step == 'name':
            ...
            context.user_data['config_description'] = text
            ...
            try:
                ...
                context.user_data['config_price_type'] = 'coins'
            ...
            if update.message.document:
                ...
                config_id = add_config(
                    ...
                    context.user_data['config_price'],
                    'coins',
                    file_path,
                ...
                await update.message.reply_text(
                    ...
                    f"ID: {config_id}\n"
                    f"Название: {context.user_data.get('config_name')}\n"
                    f"Цена: {context.user_data.get('config_price')} монет"
                )
            else:
